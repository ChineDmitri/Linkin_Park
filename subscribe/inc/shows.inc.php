...
do{ // Используется для обработки и показа ошибок
...
if($allow_add_comment){
	...
	if( strlen($name) > 50 ){
   	    echo"<div style=\"text-align: center;\">Ваше имя слишком длинное!</div>";
		$CN_HALT = TRUE;
		break 1;
	}
	if( strlen($mail) > 50){
   	    echo"<div style=\"text-align: center;\">Ваш почтовый адрес слишком большой!</div>";
		$CN_HALT = TRUE;
		break 1;
	}
	if( strlen($comments) > $config_comment_max_long and $config_comment_max_long != "" and $config_comment_max_long != "0"){
   	    echo"<div style=\"text-align: center;\">Ваш комментарий слишком длинный!</div>";
		$CN_HALT = TRUE;
		break 1;
	}
    ...
    if($config_flood_time != 0 and $config_flood_time != "" ){
        if(flooder($ip, $id) == TRUE ){
        	echo("<div style=\"text-align: center;\">Активирована защита от флуда !!!<br />вы должны подождать $config_flood_time секунд после вашего последнего комментария для отправки следующего сообщения.</div>");
			 $CN_HALT = TRUE;
             break 1;
		}
    }
    ...
    fclose($new_ips);
    if($blockip){
    	echo("<div style=\"text-align: center;\">Извините но вам запрещено добавлять комментарии</div>");
	 $CN_HALT = TRUE;
     break 1;
    }

    //----------------------------------
    // Проверка: если имя занято
    //----------------------------------
    $is_member = FALSE;
    foreach($all_users as $member_db_line)
    {
        if(!eregi("<\?",$member_db_line) and $member_db_line != ""){
            ...
            if((strtolower($user_arr[2]) == strtolower($name) or strtolower($user_arr[4]) == strtolower($name)) and    ($user_arr[3] != $CNpass and $user_arr[3] != md5($password))    and $name != "")
        	{
             ...
             echo"<div style=\"text-align: center;\">Это имя зарегистрировано другим пользователем, для его использования введите пароль<br />
             ...
             $user_post_query
             <input type=\"submit\" /> \n <br>
             <input type=\"checkbox\" name=\"CNrememberPass\" value=1 /> Запомнить пароль (md5 шифровка)
             </form>
              </div>";
			 $CN_HALT = TRUE;
             break 2;
	        ...
	        //----------------------------------
	        // Пользователь захотел запомнить пароль
	        //----------------------------------
	            if($CNrememberPass == 1){
    ...
    //----------------------------------
    // Если только зарегистрированые пользователи могут оставлять комментарии
    //----------------------------------
	if($config_only_registered_comment == "yes" and !$is_member){
	    echo"<div style=\"text-align: center;\">Извините, но только зарегистрированые пользователи могут оставлять комментарии, и имя'".htmlspecialchars($name)."' не зарегистрировано.</div>";
			 $CN_HALT = TRUE;
             break 1;
    }

    //----------------------------------
    // Wrap the long words        \\ Надоело переводить комментарии, я думаю это не нужно!
    //----------------------------------
    ...
    if($name == " " or $name == ""){
    	echo("<div style=\"text-align: center;\">Вы должны ввести имя.<br /><a href=\"javascript:history.go(-1)\">вернуться</a></div>");
		$CN_HALT = TRUE;
		break 1;
	}
    if($mail == " " or $mail == ""){ $mail = "none"; }
        ...
        elseif($config_allow_url_instead_mail != "yes"){
        	echo("<div style=\"text-align: center;\">Это не правильный почтовый адрес<br /><a href=\"javascript:history.go(-1)\">вернуться</a></div>");
			$CN_HALT = TRUE;
			break 1;
        }
		else{
        	echo("<div style=\"text-align: center;\">Это не правильный почтовый адрес или адрес сайта<br /><a href=\"javascript:history.go(-1)\">вернуться</a></div>");
			$CN_HALT = TRUE;
			break 1;
        }
    }
    ...
    if($comments == ""){
    	echo("<div style=\"text-align: center;\">Сообщение не должно быть пустым<br /><a href=\"javascript:history.go(-1)\">вернуться</a></div>");
			$CN_HALT = TRUE;
			break 1;
    }
    ...
    if(!$found){/* // Не добавлять комментарий если номер новости не найден \\ fwrite($new_comments, "$id|>|$time|$name|$mail|$ip|$comments||\n");*/ }
...
if($allow_full_story){
	...
	if(!$found){
    	echo("<div style=\"text-align: center;\">Нельзя найти статью по её номеру (ID): <strong>". @(int) htmlspecialchars($id)."</strong></div>");
		$CN_HALT = TRUE;
		break 1;
    }
}
...
if($allow_comments){
	...
	//]]></script>
	<noscript>В вашем броузере не включена поддержка Javascript. Мы рекомендуем включить её, при использовании Javascript ввод данных упрощается.
...
// Archives Looop
        if($i < $number and $only_active != TRUE){
			...
			if(!$handle = opendir("$cutepath/data/archives")){ die("<div style=\"text-align: center;\">Не возможно открыть папку $cutepath/data/archives</div>"); }
