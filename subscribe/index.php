...
/***************************************************************************
 ...
 Copyright (C) 2004 Georgi Avramov  (flexer@gmail.com)
...
if($action == "logout")
{
    ...
    msg("info", "Выход", "Вы вышли, <a href=\"$PHP_SELF\">Войти снова</a><br /><br>");
}
...
if(isset($username))
{
    ...
    if(check_login($username, $cmd5_password))
    {
    	...
    	$result = "<font color=red>Не правильный логин или пароль</font>";
        $cookie_logged = FALSE;
   }
}
...
if($action == "dologin")
{
    ...
    if(check_login($username, $md5_password)){
		...
		$result = "<font color=red>Не правильный логин и/или пароль</font>";
		$session_logged = FALSE;
	}
    ...
    if(check_login($_SESSION['username'], $_SESSION['md5_password'])){
        if($_SESSION['ip'] != $ip){ $session_logged = FALSE; $result = "IP сессии не соответствует вашему IP"; }
...
if($is_loged_in == FALSE)
{
    ...
    setcookie("md5_password","");
    setcookie("login_referer","");
	echoheader("user","Пожалуйста авторизуйтесь");
...
if($config_check_referer == TRUE){
    ...
    if(!eregi("$self",$HTTP_REFERER) and $HTTP_REFERER != ""){
    	die("<h2>Извините, у вас нет доступа к этой странице !</h2><br>Попробуйте <a href=\"?action=logout\">выйти</a> и войти снова<br>Для отключения проверки измените \$config_check_referer в index.php на FALSE");
	}
}
// ********************************************************************************
...
if($HTTP_SERVER_VARS['QUERY_STRING'] == "debug"){ debug(); }
    ...
    elseif( $system_modules[$mod] )
    {
        ...
        elseif($system_modules[$mod] == "admin" and $member_db[1] != 1){ msg("error", "Доступ закрыт", "К этому модулю доступ имеет только администратор"); exit;}
        else{ die("Доступ к модулю должен быть установлен для <b>пользователя</b> или <b>администратора</b>"); }
    }
    else{ die("$mod - не правильный модуль"); }
}

echo"<!-- время выполнения: ".$Timer->stop()." -->";
